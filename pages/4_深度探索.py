...
with st.expander("查看详细数据清洗报告", expanded=False):
    st.markdown("""
    ### 📊 清洗统计
    - **原始数据**: 400 条
    - **清洗后保留**: 341 条 (85.25%)
    ...
    ### 🚫 主要异常类型
    1. **逻辑冲突**: 年龄<30岁但已退休 (24例)
    2. **数据矛盾**: 高压力(>9)但睡眠质量极好(>8) (17例)
    3. **极端异常**: 低压力(<2)但睡眠质量极差(<4) (12例)
    
    ### ✅ 结论
    清洗后的数据集剔除了明显的逻辑矛盾和反直觉样本，提高了分析的可信度。本页面展示的数据分析基于**原始完整数据**，以全面展示所有样本特征。
    """)
